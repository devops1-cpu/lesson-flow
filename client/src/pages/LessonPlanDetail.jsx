...
const LessonPlanDetail = () => {
  ...
  const fetchPlan = async () => {
    try {
      ...
      if (res.data.readinessAssessment?.sectionAnalysis) {
        try {
          const analysis =
            typeof res.data.readinessAssessment.sectionAnalysis === "string"
          ...
          console.error("Error parsing section analysis", e);
        }
      }

      // Fetch comments
      api
        .get(`/comments/plan/${id}`)
        .then((c) => setComments(c.data))
        .catch(console.error);
  ...
  const handleCopyPlan = async (targetClassId) => {
    setCopying(true);
    try {
      const res = await api.post(`/lesson-plans/${id}/copy`, {
    ...
    api
      ...
      .then((res) => {
        ...
        const owned = res.data.filter(
          (c) => c.ownerId === user.id && c.id !== plan.classId,
        );
  ...
  const openAssignModal = async () => {
    ...
    try {
      ...
      const validSlots = (res.data.slots || []).filter(
        (s) => s.classId === plan.classId,
      );
  ...
  const handleAssignSlot = async (slotId) => {
    ...
    try {
      await api.patch(`/timetable/slots/${slotId}/link`, {
  ...
  const canEdit =
    user?.role === "ADMIN" ||
    (user?.role === "TEACHER" && plan.teacherId === user?.id);
  ...
  const getScoreBg = (s) =>
    s >= 80 ? "#ecfdf5" : s >= 70 ? "#fffbeb" : "#fef2f2";
      ...
      <Header title="Lesson Plan">
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          {isOwnPlan && (
            <button
              className="btn btn-ghost"
              style={{ borderColor: "#8430ce", color: "#8430ce" }}
              onClick={openCopyModal}
              id="copy-to-section-btn"
            >
            ...
            <div className="modal-body">
              {teacherClasses.length === 0 ? (
                ...
                >
                  ...
                  >
                    Select a target class/section. The plan will be copied as a
                    new Draft.
                  </p>
                  {teacherClasses.map((cls) => (
                    <button
                      key={cls.id}
                      onClick={() => handleCopyPlan(cls.id)}
                      disabled={copying}
                      style={{
                        display: "flex",
                        alignItems: "center",
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 40,
                          borderRadius: 10,
                          background: cls.coverColor || "#1a73e8",
                          display: "flex",
                          alignItems: "center",
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: "#6b7280" }}>
                          {cls.subject && `${cls.subject} · `}Grade {cls.grade}
        ...
        <div className="detail-page animate-in">
          ...
          {isOwnPlan && (
            <div
              style={{
                ...
                borderRadius: 12,
                marginBottom: 16,
                background: hasReadiness
                  ? getScoreBg(readinessScore)
                  : "#f9fafb",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#6b7280" }}>
                      ...
                      {readinessCleared ? "Passed" : "Failed"}
                    </div>
                  </div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span
                    ...
                    style={{
                      fontSize: 20,
                      color: "#9ca3af",
                      transform: showReadinessDetails
                        ? "rotate(180deg)"
                        : "rotate(0deg)",
                  ...
                  {readiness.questions &&
                    (() => {
                      ...
                      const aMap = readiness.answers
                        ? typeof readiness.answers === "string"
                          ? JSON.parse(readiness.answers)
                          : readiness.answers
                        : {};
                      // qList is an ARRAY of {id, question, category, weight}
                      const questionsArray = Array.isArray(qList)
                        ? qList
                        : Object.values(qList);
                      return (
                        <div
                            ...
                            return (
                              ...
                              >
                                ...
                                <div
                                  style={{
                                    fontSize: 13,
                                    color: answer ? "#4b5563" : "#9ca3af",
                                    paddingLeft: 12,
                                    borderLeft: `3px solid ${answer ? "#1a73e8" : "#d1d5db"}`,
                                    lineHeight: 1.6,
          ...
          {["SUBMITTED", "CHANGES_REQUESTED", "APPROVED"].includes(
            ...
            >
              <span
                className="material-icons-outlined"
                style={{ color: sc.color, fontSize: 24, marginTop: 1 }}
              >
                {sc.icon}
              </span>
              <div style={{ flex: 1 }}>
                <div
                  style={{ fontWeight: 600, color: sc.color, marginBottom: 2 }}
                >
                  {sc.label}
                </div>
                ...
                <div key={section.title} className="detail-section">
                  ...
                  >
                      ...
                      .map((c) => (
                        ...
                        >
                          <img
                            src={
                              c.user.avatar ||
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(c.user.name)}&background=random`
                            }
                            alt={c.user.name}
                            style={{
                              width: 28,
                    ...
                    {activeCommentSection === section.key ? (
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            ...
                            <button
                              onClick={() => handlePostComment(section.key)}
                              style={{
                                padding: "6px 16px",
                                background: "#1a73e8",
                                color: "#fff",
                      ...
                      <button
                        onClick={() => setActiveCommentSection(section.key)}
                        style={{
                          display: "flex",
                          alignItems: "center",
                          gap: 6,
